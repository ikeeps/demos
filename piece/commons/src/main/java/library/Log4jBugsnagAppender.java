...
/**
 *
 * Bugsnag appender for log4j
 * Mostly copy from Bugsnag logback
 *
 * @author hujianfeng
 * @date 2019/12/11
 *
 */
public class Log4jBugsnagAppender extends AppenderSkeleton implements Appender {
    ...
    public Log4jBugsnagAppender(Bugsnag bugsnag) {
        super();
     ...
     */
    private void populateContextData(Report report, LoggingEvent event) {
     ...
     */
    private Severity calculateSeverity(LoggingEvent event) {
     ...
     */
    private Throwable extractThrowable(LoggingEvent event) {
    ...
    private Bugsnag createBugsnag() {
        Bugsnag client = new Bugsnag(apiKey, false);
        ...
        if (endpoint != null) {
            client.setEndpoints(endpoint, null);
        }
        ...
        if (!filteredProperties.isEmpty()) {
            client.setFilters(filteredProperties.toArray(new String[0]));
        }

        client.setIgnoreClasses(ignoredClasses.toArray(new String[0]));

        if (!notifyReleaseStages.isEmpty()) {
            client.setNotifyReleaseStages(notifyReleaseStages.toArray(new String[0]));
        }

        client.setProjectPackages(projectPackages.toArray(new String[0]));
        client.setSendThreads(sendThreads);

        // Add a callback to put global meta data on every report
        client.addCallback(report -> {
